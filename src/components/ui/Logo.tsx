...
export function Logo({ inverse = false }: { inverse?: boolean }) {
  ...
  const targetHref = user ? `/${user.role}/dashboard` : '/';

  return (
    <Link href={targetHref} className="inline-flex min-h-11 items-center gap-3 rounded-lg" aria-label="ก้าวใหม่ หน้าแรก">
