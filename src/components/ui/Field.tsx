...
export function TextareaField({ label, error, id, className = '', ...props }: Common & TextareaHTMLAttributes<HTMLTextAreaElement>) {
  ...
  return <label htmlFor={inputId} className="grid min-w-0 gap-1.5 text-sm font-semibold text-slate-800">
    <span>{label}</span>
    <textarea id={inputId} aria-invalid={Boolean(error)} className={`min-h-28 min-w-0 w-full rounded-[12px] border border-slate-300 bg-white px-3.5 py-3 text-base text-slate-950 placeholder:text-slate-600 focus:border-brand-600 focus:outline-none focus:ring-4 focus:ring-brand-100 ${className}`} {...props} />
