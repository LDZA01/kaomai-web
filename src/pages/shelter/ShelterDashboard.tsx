  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      >
        ...
        <p className="relative text-sm font-bold uppercase tracking-widest" style={{ color: '#93c5fd' }}>
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <Card className="flex flex-col">
          ...
          <p className="mt-3 text-3xl font-extrabold" style={{ color: '#173A5E' }}>
            {loading ? <Loader2 size={28} className="animate-spin" /> : residents.length}
          </p>
          <p className="mt-1 text-sm text-slate-500">โปรไฟล์คนไร้บ้านที่ใช้งานอยู่</p>
        </Card>
        <Card className="flex flex-col">
          <div className="flex items-center justify-between">
          ...
          <p className="mt-3 text-3xl font-extrabold text-amber-600">
            {loading ? <Loader2 size={28} className="animate-spin" /> : pendingCount}
          </p>
        ...
        <Card className="flex flex-col">
          ...
          <p className="mt-3 text-3xl font-extrabold" style={{ color: '#0d9488' }}>
            {loading ? <Loader2 size={28} className="animate-spin" /> : hiredCount}
          </p>
      ...
      <section className="grid gap-4 lg:grid-cols-[1fr_300px]">
              ...
              {recentMatches.map((match) => (
                <div key={match.id} className="flex items-center justify-between gap-4 py-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-bold text-slate-900 text-sm">{match.resident?.name ?? 'ไม่ระบุชื่อ'}</p>
                      <p className="text-xs text-slate-500">{match.job?.title ?? 'ตำแหน่งงาน'}</p>
                    </div>
                  </div>
        ...
        <Card title="ทำสิ่งต่อไป" description="เข้าถึงฟังก์ชันหลักได้ทันที">
          <div className="mt-4 grid gap-3">
            <Link
              className="group flex items-center justify-between rounded-xl px-4 py-3.5 text-sm font-bold text-white shadow-md transition-all"
            ...
            <Link
              className="group flex items-center justify-between rounded-xl px-4 py-3.5 text-sm font-bold transition-all"
